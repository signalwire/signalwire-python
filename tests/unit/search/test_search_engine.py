...
class TestSearchEngineVectorSearch:
    ...
    def test_vector_search_success(
        self, mock_cosine_sim: MagicMock, mock_np: MagicMock
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_vector_search_database_error(
        self, mock_cosine_sim: MagicMock, mock_np: MagicMock
    ) -> None:
...
class TestSearchEngineHybridSearch:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_with_numpy_available(
        self, mock_cosine_sim: MagicMock, mock_np: MagicMock
    ) -> None:
    ...
    def test_search_with_tags_filter(self) -> None:
        ...
        engine._keyword_search_only.assert_called_once_with(
            "test query", 2, ["python"], None
        )
        assert len(results) == 1
...
class TestSearchEngineUtilities:
    ...
    def setup_method(self) -> None:
        ...
        # Insert test data
        cursor.execute(
            "INSERT INTO chunks (content, filename, language) VALUES (?, ?, ?)",
            ("Content 1", "file1.md", "en"),
        )
        cursor.execute(
            "INSERT INTO chunks (content, filename, language) VALUES (?, ?, ?)",
            ("Content 2", "file1.md", "en"),
        )
        cursor.execute(
            "INSERT INTO chunks (content, filename, language) VALUES (?, ?, ?)",
            ("Content 3", "file2.py", "en"),
        )
    ...
    def test_merge_results(self) -> None:
        ...
        assert (
            merged[0]["score"] > 0.7
        )  # Combined vector + keyword score (0.7*0.7 + 0.3*0.8 = 0.73)
...
def _create_full_test_db(
    ...
    cursor.execute(
        "INSERT INTO config (key, value) VALUES ('embedding_dimensions', '4')"
    )
    ...
    default_chunks = [
        {
            ...
            "metadata_text": "python tutorial beginner Tutorial Python SDK"
            if with_metadata_text
            else None,
        },
        {
            ...
            "metadata_text": "python advanced code Code Examples Python SDK"
            if with_metadata_text
            else None,
        },
        {
            ...
            "metadata_text": "javascript async Guide JS SDK"
            if with_metadata_text
            else None,
        },
        {
            ...
            "metadata": json.dumps(
                {
                    ...
                    "product": "AI Agents SDK",
                    "description": "Quickstart guide for AI Agents",
                }
            ),
            "language": "en",
            "processed_content": "getting started signalwire ai agents sdk",
            "metadata_text": "signalwire getting-started Getting Started AI Agents SDK"
            if with_metadata_text
            else None,
        },
        {
            ...
            "metadata": json.dumps(
                {"category": "Code Examples", "product": "REST API"}
            ),
            "language": "en",
            "processed_content": "code examples rest api integration",
            "metadata_text": "code api example Code Examples REST API"
            if with_metadata_text
            else None,
        },
        {
            ...
            "metadata_text": "swml reference Reference SWML"
            if with_metadata_text
            else None,
        },
    ]
...
class TestSearch:
    ...
    def test_search_returns_results(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_merges_multiple_sources(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
        ...
        many = [
            {
                ...
                "score": 1.0 - i * 0.01,
                "search_type": "vector",
                "metadata": {"filename": f"f{i}.md", "tags": []},
            }
            for i in range(10)
        ]
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_boosts_exact_matches(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
            ...
            return_value=[  # type: ignore[method-assign]  # mock
                {
                    ...
                    "score": 0.5,
                    "search_type": "vector",
                    "metadata": {"filename": "a.md", "tags": [], "metadata": {}},
                },
                {
                    "id": 2,
                    ...
                    "score": 0.6,
                    "search_type": "vector",
                    "metadata": {"filename": "b.md", "tags": [], "metadata": {}},
                },
            ]
        )
        engine._filename_search = Mock(return_value=[])  # type: ignore[method-assign]  # mock
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_handles_vector_conversion_error(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_applies_diversity_penalties(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_sets_score_field(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_similarity_threshold_filters(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
        ...
        results = engine.search(
            [0.9, 0.1, 0.0, 0.0], "test", count=5, similarity_threshold=0.5
        )
...
class TestVectorSearch:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_skips_empty_embeddings(
        self, mock_cosine: MagicMock, mock_np: MagicMock, db_no_embeddings: str
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_handles_embedding_processing_error(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
    ...
    def test_db_error_returns_empty(
        self, mock_cosine: MagicMock, mock_np: MagicMock
    ) -> None:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_search_type_is_vector(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
...
class TestKeywordSearch:
    ...
    def test_original_query_passed_to_fallback(self, full_db: str) -> None:
        ...
        results = engine._keyword_search(
            "python", count=5, original_query="python programming"
        )
        assert len(results) >= 1
...
class TestResultProcessing:
    ...
    def test_merge_sorted_by_score(self, tmp_path: Path) -> None:
        ...
        engine = self._make_engine(tmp_path)
        v = [
            {"id": 1, "content": "A", "score": 0.3, "metadata": {}},
        ...
        results = [
            {
                ...
                "score": 0.5,
                "final_score": 0.5,
                "metadata": {"filename": "code_example.py"},
            },
        ]
    ...
    def test_boost_getting_started(self, tmp_path: Path) -> None:
        ...
        results = [
            {
                "id": 1,
                "content": "how to start building agents",
                "score": 0.5,
                "final_score": 0.5,
    ...
    def test_boost_empty_query_no_change(self, tmp_path: Path) -> None:
        ...
        engine = self._make_engine(tmp_path)
        results = [
            {"id": 1, "content": "x", "score": 0.5, "metadata": {"filename": "a.md"}}
        ]
...
class TestEdgeCases:
    ...
    def test_init_pgvector_no_connection_raises(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="connection_string and collection_name are required"
        ):
...
class TestKeywordWeightDeprecated:
    ...
    @patch("signalwire.search.search_engine.np")
    @patch("signalwire.search.search_engine.cosine_similarity")
    def test_warns_and_ranks_the_same(
        self, mock_cosine: MagicMock, mock_np: MagicMock, full_db: str
    ) -> None:
        import warnings
        ...
        with pytest.warns(DeprecationWarning, match="keyword_weight"):
            weighted = engine.search(
                [0.9, 0.1, 0.0, 0.0], "python tutorial", count=3, keyword_weight=0.9
            )

        assert [(r["id"], r["score"]) for r in weighted] == [
            (r["id"], r["score"]) for r in baseline
        ]
