...
class TestPromptObjectModelBasics:
    def test_empty_pom_has_no_sections(self) -> None:
        pom = PromptObjectModel()
        assert pom.sections == []

    def test_add_section_returns_section_instance(self) -> None:
        pom = PromptObjectModel()
        section = pom.add_section(title="Greeting")
        assert isinstance(section, Section)
        assert section.title == "Greeting"

    def test_add_section_appears_in_sections(self) -> None:
        pom = PromptObjectModel()
    ...
    def test_find_section_returns_match(self) -> None:
        pom = PromptObjectModel()
    ...
    def test_find_section_returns_none_when_absent(self) -> None:
        pom = PromptObjectModel()
        assert pom.find_section("Nope") is None

    def test_render_markdown_includes_title_and_body(self) -> None:
        pom = PromptObjectModel()
    ...
    def test_render_xml_returns_xml_string(self) -> None:
        pom = PromptObjectModel()
    ...
    def test_to_json_returns_string_with_section_title(self) -> None:
        pom = PromptObjectModel()
        pom.add_section(title="A", body="body-A")
        as_json = pom.to_json()
        assert "A" in as_json
...
class TestSectionBasics:
    ...
    def test_section_add_body_replaces(self) -> None:
    ...
    def test_section_add_bullets_appends(self) -> None:
        s = Section(title="X")
    ...
    def test_section_add_subsection_returns_section(self) -> None:
    ...
    def test_section_render_markdown_includes_body_and_bullets(self) -> None:
...
class TestPromptObjectModelYaml:
    ...
    def test_to_yaml_returns_string_with_section_title(self) -> None:
        pom = PromptObjectModel()
        pom.add_section(title="Greeting", body="Hello")
        y = pom.to_yaml()
    ...
    def test_from_yaml_accepts_dict_input(self) -> None:
        # The Python contract accepts either a YAML string OR a parsed dict.
        data = [{"title": "B", "body": "y"}]
        pom = PromptObjectModel.from_yaml(data)  # type: ignore[arg-type]  # intentional: list input
