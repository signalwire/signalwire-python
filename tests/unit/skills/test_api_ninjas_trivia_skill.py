...
def _make_skill(params: dict[str, Any] | None = None) -> ApiNinjasTriviaSkill:
    """
    ...
    return ApiNinjasTriviaSkill(agent=mock_agent, params=default_params)
...
class TestApiNinjasTriviaSkillInit:
    ...
    def test_default_categories_all(self) -> None:
        skill = _make_skill()
...
class TestApiNinjasTriviaSkillGetTools:
    ...
    def test_tool_description_includes_tool_name(self) -> None:
        skill = _make_skill({"tool_name": "get_science_trivia"})
        tool = skill.get_tools()[0]
        assert "get science trivia" in tool["description"]

    def test_tool_has_category_parameter(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
    ...
    def test_tool_category_enum_matches_configured_categories(self) -> None:
    ...
    def test_tool_category_is_required(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
    ...
    def test_tool_webhook_url_correct(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
    ...
    def test_tool_webhook_includes_api_key_header(self) -> None:
        skill = _make_skill({"api_key": "my-secret-key"})
        tool = skill.get_tools()[0]
        webhook = tool["data_map"]["webhooks"][0]
        assert webhook["headers"]["X-Api-Key"] == "my-secret-key"

    def test_tool_data_map_has_error_keys(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
        assert tool["data_map"]["error_keys"] == ["error"]

    def test_tool_data_map_has_fallback_output(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
        output = tool["data_map"]["output"]
        assert isinstance(output, dict)

    def test_tool_webhook_output_is_dict(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
        webhook = tool["data_map"]["webhooks"][0]
        assert isinstance(webhook["output"], dict)

    def test_all_categories_default_enum(self) -> None:
        skill = _make_skill()
        tool = skill.get_tools()[0]
        enum_values = tool["parameters"]["properties"]["category"]["enum"]
        assert len(enum_values) == 14

    def test_category_description_includes_human_readable(self) -> None:
...
class TestApiNinjasTriviaSkillParameterSchema:
    ...
    def test_schema_has_api_key(self) -> None:
    ...
    def test_api_key_has_env_var(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
        assert schema["api_key"]["env_var"] == "API_NINJAS_KEY"

    def test_schema_has_categories(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
        assert "categories" in schema

    def test_categories_type_is_array(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
        assert schema["categories"]["type"] == "array"

    def test_categories_not_required(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
    ...
    def test_schema_has_swaig_fields(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
        assert "swaig_fields" in schema

    def test_schema_has_tool_name_for_multi_instance(self) -> None:
        schema = ApiNinjasTriviaSkill.get_parameter_schema()
