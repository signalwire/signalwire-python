...
def test_top_level_list_skills_returns_real_inventory() -> None:
    ...
    assert len(skills) > 0, (
        "list_skills() must return the skill inventory, not raise/empty"
    )
    names = {s["name"] for s in skills}
    # core built-in skills must be discoverable
    assert {"datetime", "math"} <= names, (
        f"expected built-ins missing from {sorted(names)}"
    )
    for s in skills:
...
def test_discover_skills_returns_inventory_not_none() -> None:
    ...
    assert discovered is not None, (
        "discover_skills() must not be a no-op returning None"
    )
