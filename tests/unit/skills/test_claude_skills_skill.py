...
def _make_skill(params: dict[str, Any] | None = None) -> ClaudeSkillsSkill:
    """
    ...
    return ClaudeSkillsSkill(agent=mock_agent, params=default_params)
...
class TestInvocationControl:
    ...
    def test_default_behavior_registers_both(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
...
class TestShellInjection:
    ...
    def test_disabled_by_default_passes_through(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_enabled_executes_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "shell-skill"
            _write_skill_md(skill_dir, "shell-skill", body="Result: !`echo hello`")
...
class TestFileDiscovery:
    ...
    def test_disabled_no_file_discovery(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_enabled_discovers_scripts_and_assets(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "file-skill"
            _write_skill_md(skill_dir, "file-skill")
    ...
    def test_files_listed_in_prompt_sections(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "file-skill"
...
class TestPromptSectionFix:
    ...
    def test_skip_prompt_returns_empty(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "my-skill"
    ...
    def test_no_skip_prompt_returns_sections(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "my-skill"
            _write_skill_md(skill_dir, "my-skill", body="Content here")
            skill = _make_skill({"skills_path": tmpdir})
...
class TestUnsupportedFeatureWarnings:
    ...
    def test_context_fork_warning(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
    ...
    def test_shell_pattern_warning_when_disabled(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
...
class TestHandlerPipeline:
    ...
    def test_variable_substitution_in_handler(self) -> None:
...
class TestParameterSchema:
    ...
    def test_schema_includes_existing_params(self) -> None:
        schema = ClaudeSkillsSkill.get_parameter_schema()
