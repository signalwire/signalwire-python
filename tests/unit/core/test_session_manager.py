...
class TestSessionManager:
    ...
    def test_initialization_with_custom_params(self) -> None:
        ...
        manager = SessionManager(
            secret_key=custom_secret, token_expiry_secs=custom_expiry
        )
...
class TestSessionManagerIntegration:
    ...
    def test_complete_token_workflow(self) -> None:
        ...
        assert call_id.startswith(
            debug_info["components"]["call_id"].replace("...", "")
        )
    ...
    def test_concurrent_sessions(self) -> None:
        ...
        for call_id, session_data in sessions.items():
            assert (
                manager.validate_token(call_id, "test_function", session_data["token"])
                is True
            )

            # Should be invalid for other call_ids
            for other_call_id in sessions:
                if other_call_id != call_id:
                    assert (
                        manager.validate_token(
                            other_call_id, "test_function", session_data["token"]
                        )
                        is False
                    )
        ...
        for i, (_call_id, _function_name, token) in enumerate(tokens[:10]):
            for j, (other_call_id, other_function_name, _) in enumerate(tokens[10:20]):
                if i != j:
                    assert (
                        manager.validate_token(
                            other_call_id, other_function_name, token
                        )
                        is False
                    )
