...
class TestAIVerbHandler:
    ...
    def test_validate_config_valid_prompt_text(self) -> None:
        ...
        config = {"prompt": {"text": "You are a helpful assistant"}}
    ...
    def test_validate_config_missing_prompt(self) -> None:
        ...
        config = {"post_prompt": {"text": "Summary"}}
    ...
    def test_build_config_with_prompt_pom(self) -> None:
        ...
        config = handler.build_config(prompt_pom=pom_data, post_prompt="Summary")
    ...
    def test_build_config_with_swaig(self) -> None:
        ...
        config = handler.build_config(prompt_text="You are helpful", swaig=swaig_data)
    ...
    def test_build_config_validation_error(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match="Either prompt_text or prompt_pom must be provided as base prompt",
        ):
    ...
    def test_build_config_conflicting_prompts(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="prompt_text and prompt_pom are mutually exclusive"
        ):
            handler.build_config(
                prompt_text="Text prompt", prompt_pom=[{"title": "POM section"}]
            )
    ...
    def test_build_config_prompt_and_contexts_combined(self) -> None:
        ...
        config = handler.build_config(
            prompt_text="Text prompt", contexts={"context1": {"steps": []}}
        )
...
class TestSWMLHandlerIntegration:
    ...
    def test_handler_with_complex_swaig(self) -> None:
        ...
        config = handler.build_config(
            prompt_text="You are a calculator and search assistant", swaig=complex_swaig
        )
...
class TestSWMLHandlerEdgeCases:
    ...
    def test_handler_with_none_values(self) -> None:
        ...
        config = handler.build_config(
            prompt_text="Test", post_prompt=None, post_prompt_url=None, swaig=None
        )
