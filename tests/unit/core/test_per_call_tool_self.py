...
class TenantAgent(AgentBase):
    def __init__(self) -> None:
        super().__init__(name="tenant", route="/agent", basic_auth=AUTH, suppress_logs=True)
        self.tenant = "none"
        self.set_dynamic_config_callback(self.configure)
        self.define_tool(
            name="registered", description="Registered method", parameters={},
            handler=self.registered, secure=False,
        )

    def configure(self, query: dict[str, Any], body: dict[str, Any], headers: dict[str, Any],
                  agent: AgentBase) -> None:
        agent.tenant = query.get("tenant", "none")  # set on the copy

    @AgentBase.tool(name="plain", description="Plain method", parameters={}, secure=False)
    ...
    def registered(self, args: dict[str, Any], raw_data: dict[str, Any]) -> FunctionResult:
        return FunctionResult(f"registered {self.tenant}")

    def labelled(self, label: str, args: dict[str, Any], raw_data: dict[str, Any]) -> FunctionResult:
...
def test_the_copy_rebinds_only_handlers_bound_to_the_agent() -> None:
    ...
    agent.define_tool(name="closure", description="Closure", parameters={}, handler=closure, secure=False)
...
async def test_a_partial_of_a_method_sees_the_per_request_configuration() -> None:
    agent = TenantAgent()
    agent.define_tool(
        name="partial", description="Partial", parameters={}, secure=False,
        handler=functools.partial(agent.labelled, "food"),
    )
    transport = httpx.ASGITransport(app=agent.get_app())
    async with httpx.AsyncClient(transport=transport, base_url="http://agent.test", auth=AUTH) as client:
        response = await client.post("/agent/swaig", params={"tenant": "acme"}, json=_body("partial"))
...
class SlottedAgent(AgentBase):
    ...
    def __init__(self) -> None:
        super().__init__(name="slotted", route="/agent", basic_auth=AUTH, suppress_logs=True)
        self.client = "api-client"
        self.set_dynamic_config_callback(lambda query, body, headers, agent: None)

    @AgentBase.tool(name="which_client", description="Client", parameters={}, secure=False)
    def which_client(self, args: dict[str, Any], raw_data: dict[str, Any]) -> FunctionResult:
...
class TestSkillsOnTheCopy:
    ...
    def _agent(self, configure: Any = None) -> AgentBase:
        agent = AgentBase(name="skills", route="/agent", basic_auth=AUTH, suppress_logs=True)
        agent.add_skill("datetime")
        agent.set_dynamic_config_callback(configure or (lambda query, body, headers, agent: None))
        return agent

    def test_the_copy_lists_the_agents_skills_without_setting_them_up_again(self) -> None:
        from signalwire.skills.datetime.skill import DateTimeSkill

        agent = self._agent()
        with patch.object(DateTimeSkill, "setup", autospec=True, return_value=True) as setup:
            copy = agent._create_ephemeral_copy()
    ...
    def test_the_callback_can_add_a_skill_the_agent_already_has(self) -> None:
        agent = self._agent(lambda query, body, headers, agent: agent.add_skill("datetime"))
        copy = agent._per_call_agent({}, {}, {})
        assert copy is not agent
        assert copy.list_skills() == ["datetime"]

    def test_removing_a_shared_skill_on_the_copy_leaves_the_agents_instance_alone(self) -> None:
        agent = self._agent()
