...
def _agent() -> AgentBase:
    return AgentBase(
        name="threads", route="/agent", basic_auth=AUTH, suppress_logs=True
    )
...
class TestToolHandlers:
    async def test_a_blocked_handler_doesnt_hold_up_other_requests(self) -> None:
        ...
        async with _client(agent) as client:
            call = asyncio.create_task(
                client.post("/agent/swaig", json=_swaig_body("slow"))
            )
            try:
    ...
    async def test_an_async_handler_still_runs_on_the_event_loop(self) -> None:
        ...
        agent = _agent()
        agent.define_tool(
            name="fast", description="fast", parameters={}, handler=fast, secure=False
        )
        async with _client(agent) as client:
    ...
    async def test_async_handlers_dont_wait_for_a_worker_thread(self) -> None:
        ...
        typed_body = {
            "function": "typed",
            "argument": {"parsed": [{"city": "Paris"}]},
            "call_id": "call-1",
        }
        async with _thread_pool_full(), _client(agent) as client:
            responses = [
                await asyncio.wait_for(
                    client.post("/agent/swaig", json=body), timeout=2
                )
        ...
        assert [response.json() for response in responses] == [
            {"response": "plain"},
            {"response": "callable"},
            {"response": "typed Paris"},
        ]

    async def test_a_cancelled_call_lets_the_handler_finish_its_shielded_cleanup(
        self,
    ) -> None:
        ...
        agent.define_tool(
            name="slow", description="slow", parameters={}, handler=slow, secure=False
        )
...
class TestPerRequestConfiguration:
    ...
    async def test_overlapping_copies_keep_their_own_configuration(self) -> None:
        ...
        for response, tenant in ((first, "first"), (second, "second")):
            ...
            assert [server["url"] for server in ai["SWAIG"]["mcp_servers"]] == [
                f"https://{tenant}.example.com/mcp"
            ]
    ...
    async def test_a_context_variable_the_callback_sets_reaches_the_tool(self) -> None:
        tenant: contextvars.ContextVar[str] = contextvars.ContextVar(
            "tenant", default="MISSING"
        )
    ...
    def test_a_verb_added_on_a_copy_stays_on_the_copy(self) -> None:
        ...
        assert copy.get_document()["sections"]["main"] == [
            {"play": {"url": "say:hello"}}
        ]
...
class TestOtherCallbacks:
    ...
    async def test_a_routing_callback_runs_in_a_worker_thread(self) -> None:
        ...
        async with _client(agent) as client:
            ...
            rendered = await client.post(
                "/agent/sip", json={"call": {"call_id": "call-1"}}
            )
    ...
    async def test_a_swml_service_routing_callback_runs_in_a_worker_thread(
        self,
    ) -> None:
        threads: list[int] = []
        ...
        async with httpx.AsyncClient(
            transport=transport, base_url="http://svc.test", auth=AUTH
        ) as client:
            redirected = await client.post("/svc/sip", json={"transfer": True})
            rendered = await client.post(
                "/svc/sip", json={"call": {"call_id": "call-1"}}
            )
        assert redirected.status_code == 307
        assert redirected.headers["location"] == "/elsewhere"
    ...
    async def test_a_context_variable_the_routing_callback_sets_reaches_on_swml_request(
        self,
    ) -> None:
        tenant: contextvars.ContextVar[str] = contextvars.ContextVar(
            "tenant", default="MISSING"
    ...
    async def test_a_swml_service_keeps_one_context_and_needs_no_thread_to_render(
        self,
    ) -> None:
        tenant: contextvars.ContextVar[str] = contextvars.ContextVar(
            "tenant", default="MISSING"
        ...
        class Service(SWMLService):
            def on_request(
                self, request_data: Any = None, callback_path: Any = None
            ) -> Any:
                seen.append(tenant.get())
                return None
    ...
    async def test_an_mcp_tool_call_runs_in_a_worker_thread(self) -> None:
        ...
        agent.define_tool(
            name="lookup", description="lookup", parameters={}, handler=lookup
        )
        async with _client(agent) as client:
            response = await client.post(
                "/agent/mcp",
...
async def test_what_the_worker_thread_sets_reaches_the_caller() -> None:
    ...
    region: contextvars.ContextVar[str] = contextvars.ContextVar(
        "region", default="unset"
    )
...
async def test_the_thread_pools_own_settings_stay_in_the_thread(
    ...
    pool_marker: contextvars.ContextVar[str] = contextvars.ContextVar(
        "pool", default="loop"
    )
    region: contextvars.ContextVar[str] = contextvars.ContextVar(
        "region", default="unset"
