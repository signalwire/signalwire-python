...
def _ensure_mock_module(
    module_path: str, attrs: dict[str, object] | None = None
) -> None:
...
_ensure_mock_module(
    "signalwire.search.index_builder",
    {
        "IndexBuilder": type("IndexBuilder", (), {}),
    },
)
_ensure_mock_module(
    "signalwire.search.search_engine",
    {
        "SearchEngine": type("SearchEngine", (), {}),
    },
)
_ensure_mock_module(
    "signalwire.search.query_processor",
    {
        "preprocess_query": lambda *a, **kw: {},
    },
)
_ensure_mock_module(
    "signalwire.search.migration",
    {
...
class TestBuildSearchMain:
    ...
    def test_partial_valid_sources(self, mock_builder_class: MagicMock) -> None:
        ...
        with (
            patch(
                "signalwire.cli.build_search.Path", side_effect=mock_path_constructor
            ),
            patch("os.path.exists", return_value=True),
            patch("builtins.print") as mock_print,
        ):
            ...
            mock_print.assert_any_call(
                "Warning: Source does not exist, skipping: ./missing"
            )
    ...
    def test_validation_failure(self, mock_builder_class: MagicMock) -> None:
        ...
        with (
            ...
            assert exc_info.value.code == 1
            mock_print.assert_any_call(
                "✗ Index validation failed: Invalid index format"
            )
...
class TestValidateCommand:
    ...
    def test_validate_nonexistent_file(self, mock_parser_class: MagicMock) -> None:
        ...
        with (
            ...
            assert exc_info.value.code == 1
            mock_print.assert_any_call(
                "Error: Index file does not exist: nonexistent.swsearch"
            )
...
class TestSearchCommand:
    ...
    def test_basic_search(self) -> None:
        ...
        with (
            ...
            patch("builtins.print") as mock_print,
            patch(
                "signalwire.search.search_engine.SearchEngine", return_value=mock_engine
            ),
            patch(
                "signalwire.search.query_processor.preprocess_query",
                return_value=mock_preprocess_return,
            ),
        ):
            search_command()
    ...
    def test_full_search_command(self) -> None:
        ...
        with (
            ...
            # Should output JSON
            printed_calls = [
                str(call) for call in mock_print.call_args_list if call.args
            ]
...
class TestArgumentParsing:
    ...
    def test_sentence_chunking_with_newlines(
...
class TestErrorHandlingEdgeCases:
    ...
    def test_validate_verbose_error_with_traceback(
        self, mock_builder_class: MagicMock
    ) -> None:
    ...
    def test_search_verbose_error_with_traceback(self) -> None:
        ...
        with (
            ...
            patch(
                "signalwire.search.search_engine.SearchEngine",
                side_effect=Exception("Search detailed error"),
            ),
            pytest.raises(SystemExit),
        ):
            search_command()
...
class TestConsoleEntryPointExtended:
    ...
    def test_console_entry_help_short_flag(
...
class TestMainPgvectorBackend:
    ...
    def test_pgvector_default_output_single_source(
        self, mock_builder_class: MagicMock
    ) -> None:
        ...
        with (
            ...
            patch("pathlib.Path.is_file", return_value=False),
            patch(
                "pathlib.Path.name", new_callable=lambda: property(lambda self: "docs")
            ),
            patch("builtins.print"),
        ):
            main()
    ...
    def test_pgvector_default_output_multi_source(
        self, mock_builder_class: MagicMock
    ) -> None:
...
class TestMainOutputConflict:
    ...
    @patch(
        "sys.argv",
        ["sw-search", "./docs", "--output", "out.swsearch", "--output-dir", "./dir"],
    )
    def test_output_and_output_dir_conflict(self) -> None:
        ...
        assert exc_info.value.code == 1
        mock_print.assert_any_call(
            "Error: Cannot specify both --output and --output-dir"
        )
...
class TestMainJsonOutputFormat:
    ...
    def test_json_format_default_output_name(
        self, mock_builder_class: MagicMock
    ) -> None:
    ...
    def test_json_format_ignores_backend_warning(
        self, mock_builder_class: MagicMock
    ) -> None:
    ...
    def test_json_format_output_gets_json_extension(
        self, mock_builder_class: MagicMock
    ) -> None:
...
class TestMainModelAlias:
    ...
    def test_model_alias_large(
        self, mock_builder_class: MagicMock, capsys: pytest.CaptureFixture[str]
    ) -> None:
...
class TestMainVerboseStrategies:
    ...
    @patch(
        "sys.argv",
        ["sw-search", "./docs", "--verbose", "--chunking-strategy", "paragraph"],
    )
    ...
    @patch("signalwire.search.index_builder.IndexBuilder")
    @patch(
        "sys.argv", ["sw-search", "./docs", "--verbose", "--chunking-strategy", "page"]
    )
    ...
    @patch(
        "sys.argv",
        ["sw-search", "./docs", "--verbose", "--chunking-strategy", "semantic"],
    )
    ...
    @patch("signalwire.search.index_builder.IndexBuilder")
    @patch(
        "sys.argv", ["sw-search", "./docs", "--verbose", "--chunking-strategy", "topic"]
    )
    ...
    @patch("signalwire.search.index_builder.IndexBuilder")
    @patch(
        "sys.argv", ["sw-search", "./docs", "--verbose", "--chunking-strategy", "qa"]
    )
    ...
    @patch(
        "sys.argv",
        ["sw-search", "./docs", "--verbose", "--chunking-strategy", "sentence"],
    )
...
class TestMainSingleFileSource:
    ...
    def test_single_file_source_names_output(
        self, mock_builder_class: MagicMock
    ) -> None:
        ...
        with (
            ...
            patch(
                "pathlib.Path.stem",
                new_callable=lambda: property(lambda self: "README"),
            ),
            patch("os.path.exists", return_value=True),
            patch("builtins.print"),
        ):
...
class TestValidateCommandExtended:
    ...
    def test_validate_success(
        self, mock_parser_class: MagicMock, mock_builder_class: MagicMock
    ) -> None:
    ...
    @patch("signalwire.search.index_builder.IndexBuilder")
    @patch("argparse.ArgumentParser")
    def test_validate_success_verbose(
        self, mock_parser_class: MagicMock, mock_builder_class: MagicMock
    ) -> None:
    ...
    def test_validate_failure(
        ...
        assert exc_info.value.code == 1
        mock_print.assert_any_call("\u2717 Index validation failed: corrupted index")
...
class TestSearchCommandExtended:
    ...
    def test_search_keyword_weight_too_high(self) -> None:
        ...
        assert exc_info.value.code == 1
        mock_print.assert_any_call(
            "Error: --keyword-weight must be between 0.0 and 1.0"
        )
...
class TestRemoteCommand:
    ...
    @patch(
        "sys.argv",
        ["remote", "http://localhost:8001/", "query", "--index-name", "docs"],
    )
    ...
    @patch(
        "sys.argv",
        ["remote", "http://localhost:8001/search", "query", "--index-name", "docs"],
    )
    ...
    @patch(
        "sys.argv", ["remote", "http://localhost:8001", "query", "--index-name", "docs"]
    )
...
class TestRemoteCredentialsAndThreshold:
    ...
    def test_user_with_password_from_environment(self) -> None:
        post, _ = self._run(
            ["http://localhost:8001", "q", "--index-name", "docs", "--user", "u"],
    ...
    def test_verbose_endpoint_is_redacted(self) -> None:
        _, out = self._run(
            ["http://u:s3cret@localhost:8001", "q", "--index-name", "docs", "--verbose"]
        )
    ...
    def test_verbose_endpoint_with_empty_user_is_redacted(self) -> None:
        _, out = self._run(
            ["http://:s3cret@localhost:8001", "q", "--index-name", "docs", "--verbose"]
        )
        assert "s3cret" not in out
        assert "localhost:8001" in out
    ...
    def test_connection_error_is_redacted(self) -> None:
        ...
        _, out = self._run(
            ["http://u:s3cret@localhost:8001", "q", "--index-name", "docs"],
            post_side_effect=real_requests.ConnectionError("refused"),
        )
    ...
    def test_threshold_under_either_name(self, flag: str) -> None:
        post, _ = self._run(
            ["http://localhost:8001", "q", "--index-name", "docs", flag, "0.7"]
        )
