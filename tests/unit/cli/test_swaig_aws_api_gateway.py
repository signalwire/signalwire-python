...
def _swaig_test(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "signalwire.cli.swaig_test_wrapper",
         str(REPO / "examples" / "simple_agent.py"), "--simulate-serverless", "lambda",
         *args, "--dump-swml", "--raw"],
...
def test_api_gateway_url_is_used_for_webhooks() -> None:
    completed = _swaig_test("--aws-api-gateway-id", "abc123", "--aws-region", "us-west-2", "--aws-stage", "dev")
