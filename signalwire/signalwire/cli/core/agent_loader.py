...
def discover_services_in_file(service_path: str) -> List[Dict[str, Any]]:
    ...
    if not SWML_SERVICE_AVAILABLE:
        raise ImportError(
            "SWMLService not available. Please install signalwire-agents package."
        )
...
def load_agent_from_file(
    ...
    if not AGENT_BASE_AVAILABLE:
        raise ImportError(
            "AgentBase not available. Please install signalwire-agents package."
        )
...
def _load_service_impl(
    ...
    # If service_identifier is specified as a class name, try to instantiate that specific class first
        ...
        if hasattr(module, service_identifier):
            ...
            if isinstance(obj, type) and issubclass(obj, SWMLService):
                ...
                raise ValueError(
                    f"'{service_identifier}' is not a valid SWMLService class or instance"
                )
        else:
            raise ValueError(
                f"Service class '{service_identifier}' not found in {service_path}"
            )
    ...
    if service is None and hasattr(module, "main"):
        print(
            "Warning: No agent instance found, attempting to call main() without server startup"
        )
        try:
    ...
    if service is None:
        raise ValueError(
            f"No service found in {service_path}. The file must contain either:\n"
