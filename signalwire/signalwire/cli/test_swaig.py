...
def main():
    ...
    parser.add_argument("agent_path", help="Path to Python file containing the agent")
    ...
    common.add_argument(
        "--raw", action="store_true", help="Output raw JSON only (for piping to jq)"
    )
    common.add_argument(
        "--agent-class",
    ...
    common.add_argument(
        "--route", help="Specify service by route (e.g., /healthcare, /finance)"
    )

    # Actions (choose one)
    actions = parser.add_argument_group("actions (choose one)")
    actions.add_argument(
        "--list-agents", action="store_true", help="List all agents in file"
    )
    actions.add_argument(
        "--list-tools", action="store_true", help="List all tools in agent"
    )
    actions.add_argument(
        "--dump-swml", action="store_true", help="Generate and output SWML document"
    )
    actions.add_argument(
        "--exec",
        metavar="FUNCTION",
    ...
    func_group.add_argument(
        "--fake-full-data", action="store_true", help="Use comprehensive fake post_data"
    )
    ...
    swml_group.add_argument(
        "--call-state", default="created", help="Call state (default: created)"
    )
    swml_group.add_argument("--from-number", help="Override from number")
    swml_group.add_argument("--to-extension", help="Override to extension")

    # Data customization
    data_group = parser.add_argument_group("data customization")
    data_group.add_argument("--user-vars", help="JSON string for userVariables")
    data_group.add_argument("--query-params", help="JSON string for query parameters")
    ...
    serverless_group = parser.add_argument_group(
        "serverless simulation (use --help-platforms for platform options)"
    )
    ...
    parser.add_argument(
        "--override-json", action="append", default=[], help=argparse.SUPPRESS
    )
        ...
        if args.env_file:
            try:
                ...
                if args.verbose and not args.raw:
                    print(
                        f"Loaded {len(file_env)} environment variables from {args.env_file}"
                    )
    ...
    try:
        ...
        if args.list_agents:
            try:
                ...
                for agent_info in agents:
                    agent_type = (
                        "instance" if agent_info["type"] == "instance" else "class"
                    )
        ...
        try:
            ...
            agent = load_service_from_file(
                args.agent_path, service_identifier, prefer_route
            )
        except ValueError as e:
            error_msg = str(e)
            if (
                "Multiple agent classes found" in error_msg
                and args.list_tools
                and not args.agent_class
            ):
                    ...
                    if agents:
                        ...
                        for agent_info in agents:
                            if agent_info["type"] == "class":
                                ...
                                if agent_info["description"]:
                                    desc = (
                                        agent_info["description"].strip().split("\n")[0]
                                    )
                                    if desc:
                                ...
                                try:
                                    specific_agent = load_agent_from_file(
                                        args.agent_path, agent_info["class_name"]
                                    )
                                    ...
                                    try:
                                        ...
                                        if args.verbose:
                                            print(
                                                f"    (Warning: Dynamic config failed: {dc_error})"
                                            )

                                    functions = (
                                        specific_agent._tool_registry.get_all_functions()
                                        if hasattr(specific_agent, "_tool_registry")
                                        else {}
                                    )
                ...
                print(
                    ERROR_AGENT_NOT_FOUND.format(
                        class_name=args.agent_class, file_path=args.agent_path
                    )
                )
            else:
        ...
        if args.tool_name:
            ...
            is_external_webhook = (
                hasattr(func, "webhook_url")
                and func.webhook_url
                and hasattr(func, "is_external")
                and func.is_external
            )
            ...
            if is_datamap:
                ...
                if args.minimal:
                    post_data = generate_minimal_post_data(
                        args.tool_name, function_args
                    )
                    if args.custom_data:
                ...
                post_data = apply_overrides(
                    post_data, args.override, args.override_json
                )
