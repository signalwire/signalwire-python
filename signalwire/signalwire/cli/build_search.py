...
Examples:
  ...
  sw-search search docs_collection "how to create an agent" \\
    ...
    parser.add_argument(
        "sources", nargs="+", help="Source files and/or directories to index"
    )
    ...
    if args.output_format == "index" and not args.output and not args.output_dir:
        if args.backend == "sqlite":
            if len(valid_sources) == 1:
                ...
                source_name = (
                    valid_sources[0].stem
                    if valid_sources[0].is_file()
                    else valid_sources[0].name
                )
    ...
    if (
        args.output_format == "index"
        and args.backend == "sqlite"
        and args.output
        and not args.output.endswith(".swsearch")
    ):
    ...
    exclude_patterns = (
        [p.strip() for p in args.exclude.split(",")] if args.exclude else None
    )
    ...
    try:
        ...
        if args.output_format == "json":
            ...
            files = builder._discover_files_from_sources(
                valid_sources, file_types, exclude_patterns
            )
            ...
            for file_path in files:
                try:
                    ...
                    base_dir = builder._get_base_directory_for_file(
                        file_path, valid_sources
                    )
...
def search_command() -> None:
    ...
    parser.add_argument(
        "index_source", help="Path to .swsearch file or collection name for pgvector"
    )
    parser.add_argument(
        "query", nargs="?", help="Search query (optional if using --shell)"
    )
    parser.add_argument(
        "--backend",
        choices=["sqlite", "pgvector"],
    ...
    parser.add_argument(
        "--no-content",
        action="store_true",
        help="Hide content in results (show only metadata)",
    )
    parser.add_argument(
        "--model",
    ...
    try:
        ...
        if args.backend == "sqlite":
            ...
            engine = SearchEngine(
                backend="sqlite", index_path=args.index_source, model=model
            )
        else:
        ...
        if not model_to_use and "config" in stats:
            ...
            model_to_use = stats["config"].get("embedding_model") or stats[
                "config"
            ].get("model_name")
        ...
        if args.shell:
            ...
            print(f"Backend: {args.backend}")
            print(
                f"Index contains {stats['total_chunks']} chunks from {stats['total_files']} files"
            )
            if model_to_use:
        ...
        if args.json:
            ...
            for i, result in enumerate(results):
                ...
                if metadata.get("line_start"):
                    print(
                        f"Lines: {metadata['line_start']}-{metadata.get('line_end', metadata['line_start'])}"
                    )
...
Examples:
  ...
  sw-search migrate --info ./docs.swsearch
    ...
    migration_group.add_argument(
        "--to-pgvector", action="store_true", help="Migrate SQLite index to pgvector"
    )
    migration_group.add_argument(
        "--to-sqlite",
        action="store_true",
    ...
    migration_group.add_argument(
        "--info", action="store_true", help="Show information about an index"
    )

    # pgvector options
    parser.add_argument(
        "--connection-string", help="PostgreSQL connection string for pgvector"
    )
    parser.add_argument("--collection-name", help="Collection name for pgvector")
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing collection"
    )

    # SQLite options
    parser.add_argument("--output", help="Output .swsearch file path (for --to-sqlite)")
    ...
    if args.info:
        ...
        try:
            ...
            if info["type"] == "sqlite":
                ...
                print(
                    f"  Dimensions: {info['config'].get('embedding_dimensions', 'Unknown')}"
                )
    ...
    try:
        ...
        if args.to_pgvector:
            ...
            # Perform migration
            print(
                f"Migrating {args.source} to pgvector collection '{args.collection_name}'..."
            )
...
def remote_command() -> None:
    ...
    parser.add_argument(
        "endpoint", help="Remote API endpoint URL (e.g., http://localhost:8001)"
    )
    parser.add_argument("query", help="Search query")
    parser.add_argument(
        "--index-name", required=True, help="Name of the index to search"
    )
    parser.add_argument(
        "--count", type=int, default=5, help="Number of results to return (default: 5)"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if args.json:
                ...
                if not results:
                    print(
                        f"No results found for '{args.query}' in index '{args.index_name}'"
                    )
                    sys.exit(0)

                print(
                    f"Found {len(results)} result(s) for '{args.query}' in index '{args.index_name}':"
                )
                if (
                    result.get("enhanced_query")
                    and result.get("enhanced_query") != args.query
                ):
