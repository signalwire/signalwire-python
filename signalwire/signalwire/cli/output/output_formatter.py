...
def display_agent_tools(agent: "AgentBase", verbose: bool = False) -> None:
    ...
    functions = {}
    if hasattr(agent, "_tool_registry") and hasattr(
                    ...
                    if properties:
                        ...
                        for param_name, param_def in properties.items():
                            ...
                            if "enum" in param_def:
                                constraints.append(
                                    f"options: {', '.join(map(str, param_def['enum']))}"
                                )
                            ...
                            if "exclusiveMinimum" in param_def:
                                constraints.append(
                                    f"min (exclusive): {param_def['exclusiveMinimum']}"
                                )
                            if "exclusiveMaximum" in param_def:
                                constraints.append(
                                    f"max (exclusive): {param_def['exclusiveMaximum']}"
                                )
                            if "multipleOf" in param_def:
                                constraints.append(
                                    f"multiple of: {param_def['multipleOf']}"
                                )

                            # String constraints
                            if "minLength" in param_def:
                                constraints.append(
                                    f"min length: {param_def['minLength']}"
                                )
                            if "maxLength" in param_def:
                                constraints.append(
                                    f"max length: {param_def['maxLength']}"
                                )
                            ...
                            if param_type == "array":
                                if "minItems" in param_def:
                                    constraints.append(
                                        f"min items: {param_def['minItems']}"
                                    )
                                ...
                                if (
                                    "items" in param_def
                                    and "type" in param_def["items"]
                                ):
                                    constraints.append(
                                        f"item type: {param_def['items']['type']}"
                                    )
