...
class InfoGathererAgent(AgentBase):
    ...
    def _generate_question_instruction(
        ...
        if is_first_question:
            instruction = (
                f"Ask the user to answer the following question: {question_text}\n\n"
            )
        else:
    ...
    def submit_answer(self, args, raw_data):
        ...
        print(
            f"new_question_index: {new_question_index} len(questions): {len(questions)}"
        )

        # Check if we have more questions
        if new_question_index < len(questions):
            print(
                f"Asking next question: {new_question_index} question: {questions[new_question_index]}"
            )
