...
class SurveyAgent(AgentBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.introduction = (
            introduction
            or f"Welcome to our {survey_name}. We appreciate your participation."
        )
        self.conclusion = (
            conclusion
            or "Thank you for completing our survey. Your feedback is valuable to us."
        )
    ...
    def _validate_questions(self):
        ...
        for i, question in enumerate(self.questions):
            ...
            if "type" not in question or question["type"] not in valid_types:
                raise ValueError(
                    f"Question {i + 1} has an invalid type. Must be one of: {', '.join(valid_types)}"
                )
            ...
            if question["type"] == "multiple_choice" and (
                "options" not in question or not question["options"]
            ):
                raise ValueError(
                    f"Multiple choice question '{question['id']}' must have options"
                )
    ...
    def _setup_survey_agent(self):
        ...
        self.prompt_add_section(
            "Introduction", body=f"Begin with this introduction: {self.introduction}"
        )
        ...
        for q in self.questions:
            ...
            questions_subsections.append({"title": q["text"], "body": description})
        ...
        self.prompt_add_section(
            "Conclusion", body=f"End with this conclusion: {self.conclusion}"
        )
        ...
        self.add_hints([self.survey_name, self.brand_name, *type_terms])
