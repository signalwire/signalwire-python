...
``namespaces.calling`` (the ``*Resource``/``*Namespace`` suffixes were dropped). This thin
re-export keeps ``from signalwire.signalwire.rest.namespaces.calling import CallingNamespace`` working
but emits a DeprecationWarning. Prefer ``client.calling`` (no import needed). PYTHON-ONLY: the
