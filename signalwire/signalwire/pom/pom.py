...
class Section:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(body, str):
            raise TypeError(
                f"body must be a string, not {type(body).__name__}. "
        ...
        if bullets is not None and not isinstance(bullets, list):
            raise TypeError(
                f"bullets must be a list or None, not {type(bullets).__name__}"
            )
    ...
    def render_xml(
        ...
        if self.title is not None:
            ...
            if (
                section_number
            ):  # If we have a section number, use it (regardless of self.numbered)
                prefix = f"{'.'.join(map(str, section_number))}. "
...
class PromptObjectModel:
    ...
    def render_markdown(self) -> str:
        ...
        for i, section in enumerate(self.sections):
            ...
            if self.debug:
                print(
                    f"Rendering section {i}: {section.title} with section_number={section_number}"
                )
