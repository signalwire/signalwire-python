...
class AgentServer:
    ...
    def register(self, agent: AgentBase, route: str | None = None) -> None:
        ...
        self.logger.info(
            "agent_registered", extra={"agent": agent.get_name(), "route": route}
        )
    ...
    def register_sip_username(self, username: str, route: str) -> None:
        ...
        if not self._sip_routing_enabled:
            self.logger.warning(
                "SIP routing is not enabled. Call setup_sip_routing() first."
            )
            return
        ...
        if route not in self.agents:
            self.logger.warning(
                f"Route {route} not found. SIP username will be registered but may not work."
            )
    ...
    def _run_server(self, host: str | None = None, port: int | None = None) -> None:
        ...
        if ssl_enabled and ssl_cert_path and ssl_key_path:
            self.logger.info(
                f"Starting with SSL - cert: {ssl_cert_path}, key: {ssl_key_path}"
            )
            uvicorn.run(
                self.app,
                host=host,
    ...
    def _serve_static_file(self, file_path: str, route: str = "/") -> Response | None:
        ...
        try:
            ...
            if (
                not str(full_path).startswith(str(static_dir) + os.sep)
                and full_path != static_dir
            ):
                return None
        except Exception:
            return None
            ...
            if hasattr(self, "_static_directories"):
                ...
                for static_route in self._static_directories:
                    ...
                    # For prefixed static routes, check if path matches
                        ...
                        static_route.lstrip("/") + "/"
                    ) or full_path == static_route.lstrip("/"):
                        relative_path = full_path[
                            len(static_route.lstrip("/")) :
                        ].lstrip("/")
