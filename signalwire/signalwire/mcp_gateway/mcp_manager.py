...
class MCPClient:
    ...
    def _sandbox_preexec(self):
        ...
        if sandbox_config.get("drop_privileges", False):
            ...
            logger.warning(
                f"Running MCP service '{self.service.name}' as root without drop_privileges enabled"
            )
    ...
    def start(self) -> bool:
        ...
        try:
            logger.info(
                f"Starting MCP service '{self.service.name}' with command: {self.service.command}"
            )
            ...
            logger.info(
                "mcp_service_started",
                extra={"service": self.service.name, "tool_count": len(self.tools)},
            )
    ...
    def stop(self):
            ...
            try:
                ...
                if self.process.poll() is None:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Force killing MCP service '{self.service.name}'"
                        )
                        self.process.kill()
                        self.process.wait(timeout=1)
        ...
        if self.reader_thread and self.reader_thread.is_alive():
            ...
            if self.reader_thread.is_alive():
                logger.warning(
                    f"Reader thread for '{self.service.name}' did not stop gracefully"
                )
    ...
    def _initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Initialized '{self.service.name}': {result.get('serverInfo', {})}"
            )
            return True
...
class MCPManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.sandbox_base_dir = config.get("session", {}).get(
            "sandbox_dir", "./sandbox"
        )
