...
class SessionManager:
    ...
    def __init__(self, config: Dict[str, Any], max_total_sessions: int = 500):
        ...
        self.max_total_sessions = config.get("session", {}).get(
        ...
        logger.info(
            f"SessionManager initialized with cleanup_interval={self.cleanup_interval}s"
        )
    ...
    def create_session(
        ...
        with self.lock:
            ...
            if session_id in self.sessions:
                logger.warning(
                    f"Session {session_id} already exists, closing old session"
                )
            ...
            if len(self.sessions) >= self.max_total_sessions:
                raise RuntimeError(
                    f"Maximum session limit ({self.max_total_sessions}) reached"
                )

            # Check service limits
            service_count = sum(
                1 for s in self.sessions.values() if s.service_name == service_name
            )
            if service_count >= self.max_sessions_per_service:
                raise RuntimeError(
                    f"Max sessions limit reached for service {service_name}"
                )
        ...
        with self.lock:
            ...
            if session.process:
                try:
                    ...
                    logger.error(
                        f"Error stopping MCP client for session {session_id}: {e}"
                    )
                ...
                result[session_id] = {
                    ...
                    "time_remaining": max(
                        ...
                        (
                            ...
                            + timedelta(seconds=session.timeout)
                            - datetime.now()
                        ).total_seconds(),
                    ),
                }
