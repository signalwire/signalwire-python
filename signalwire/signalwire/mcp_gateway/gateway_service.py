...
from signalwire.mcp_gateway.session_manager import SessionManager
from signalwire.mcp_gateway.mcp_manager import MCPManager
...
class MCPGateway:
    ...
    def __init__(self, config_path: str = "config.json") -> None:
        ...
        if self.config_loader.has_config():
            ...
            self.config = self.config_loader.substitute_vars(
    ...
    def _load_config(self, config_path: str) -> dict[str, Any]:
        ...
        if isinstance(config.get("server", {}).get("port"), str):
            try:
                ...
                logger.warning(
                    f"Invalid port value: {config['server']['port']}, using default 8080"
                )
        ...
        for key in ["default_timeout", "max_sessions_per_service", "cleanup_interval"]:
            if key in session_config and isinstance(session_config[key], str):
                try:
                    ...
                    logger.warning(
                        f"Invalid {key} value: {session_config[key]}, using default"
                    )
        ...
        def call_service_tool(service_name: str) -> Any:
            ...
            try:
                ...
                if not session:
                    # Create new session
                    logger.info(
                        f"Creating new session {session_id} for service {service_name}"
                    )
                ...
                # Call the tool
                logger.info(
                    f"Calling {service_name}.{tool_name} for session {session_id}"
                )
    ...
    def run(self) -> None:
        ...
        self.server = make_server(
            host, port, self.app, ssl_context=ssl_context, threaded=True
        )
