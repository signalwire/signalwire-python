...
class SearchService:
    ...
    def _load_config(self, config_file: Optional[str]):
        ...
        if service_config:
            ...
            if "indexes" in service_config and isinstance(
                service_config["indexes"], dict
            ):
    ...
    def _setup_routes(self):
        ...
        async def health():
            return {
                ...
                "connection_string": "***"
                if self.backend == "pgvector" and self.connection_string
                else None,
            }
                ...
                try:
                    ...
                    if model_name:
                        ...
                        if model_name not in self.models:
                            ...
                            logger.info(
                                f"Using cached model {model_name} for collection {collection_name}"
                            )
                    else:
                        logger.warning(
                            f"No model_name in config for collection {collection_name}"
                        )
    ...
    async def _handle_search(self, request: SearchRequest) -> SearchResponse:
        ...
        if self.backend == "pgvector" and hasattr(self, "collection_models"):
            ...
            if collection_model_name and collection_model_name in self.models:
                ...
                logger.debug(
                    f"Set global model to {collection_model_name} for collection {request.index_name}"
                )
        ...
        if hasattr(search_engine, "config") and search_engine.config:
            ...
            model_name = search_engine.config.get(
                "model_name"
