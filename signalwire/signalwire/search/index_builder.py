...
class IndexBuilder:
    ...
    def __init__(
        ...
        if self.backend not in ["sqlite", "pgvector"]:
            raise ValueError(
                f"Invalid backend '{self.backend}'. Must be 'sqlite' or 'pgvector'"
            )

        # Validate NLP backend
        if self.index_nlp_backend not in ["nltk", "spacy"]:
            logger.warning(
                f"Invalid index_nlp_backend '{self.index_nlp_backend}', using 'nltk'"
            )
    ...
    def build_index_from_sources(
        ...
        if not files:
            print(
                "No files found to process. Check your sources, file types and exclude patterns."
            )
            return
        ...
        if not chunks:
            print(
                "No chunks created from documents. Check file contents and processing."
            )
            return
        ...
        if self.backend == "sqlite":
            ...
            self._create_database(
                output_file, chunks, languages or ["en"], sources_info, file_types
            )
            ...
            self._store_chunks_pgvector(
                chunks, output_file, languages or ["en"], overwrite
            )
        ...
        for source in sources:
            if source.is_file():
                ...
                if (
                    ...
                    if self.verbose:
                        print(
                            f"Skipped unsupported file type: {source} (extension: {file_ext})"
                        )

            elif source.is_dir():
                # Directory - use existing discovery logic
                dir_files = self._discover_files(
                    str(source), file_types, exclude_patterns
                )
    ...
    def _process_file(
        ...
        try:
            ...
            if file_extension == ".pdf":
                ...
                content_result = self.doc_processor._extract_text_from_file(
                    str(file_path)
                )
                if isinstance(content_result, str) and content_result.startswith(
                    '{"error"'
                ):
                    if self.verbose:
                ...
                if isinstance(content_result, str) and content_result.startswith(
                    ...
                    if self.verbose:
                        print(
                            f"Skipping office document (extraction failed): {file_path}"
                        )
                    return []
                content = content_result
    ...
    def _create_database(
        ...
        try:
            ...
            config_data = {
                ...
                "sources": json.dumps(
                    sources_info
                ),  # Store list of sources instead of single directory
            ...
            for chunk in chunks:
                ...
                json_metadata, json_metadata_text = (
                    self._extract_metadata_from_json_content(chunk["content"])
                )
