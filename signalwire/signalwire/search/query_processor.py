    ...
    try:
        ...
        try:
            ...
            if not _spacy_warning_shown:
                logger.warning(
                    f"spaCy model '{model_name}' not found. Falling back to NLTK."
                )
...
def set_global_model(model):
    ...
    if model:
        ...
        if model_name:
            with _model_lock:
                if (
                    model_name not in _model_cache
                    and len(_model_cache) >= _MAX_MODEL_CACHE_SIZE
                ):
...
def preprocess_query(
    ...
    if pos_to_expand is None:
        pos_to_expand = [
            ...
            "VERB",
            "ADJ",
        ]  # Default to expanding synonyms for nouns, verbs, and adjectives
    ...
    if query_nlp_backend == "spacy":
        ...
        if nlp is None and debug:
            logger.info(
                "spaCy backend requested but not available, falling back to NLTK"
            )
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"NLTK tokenization fallback failed: {fallback_error}. Using simple word splitting."
            )
            tokens = query.split()
    ...
    if nlp and query_nlp_backend == "spacy":
        ...
        try:
            ...
            logger.warning(
                f"NLTK POS tagging failed: {pos_error}. Using basic token processing."
            )
            for token in tokens:
                lemma = token.lower()
                stemmed = stemmer.stem(lemma)
    ...
    for original, lemma in lemmas:
        ...
        if (
            lemma not in expanded_query_set and not preserve_original
        ):  # Only add lemmas if not preserving original
    ...
    formatted_output: Dict[str, Any] = {
        ...
        "language": language,
        "POS": pos_tags,
        "nlp_backend_used": query_nlp_backend
        if nlp or query_nlp_backend == "nltk"
        else "nltk",
    }
