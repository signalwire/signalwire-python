...
class DataSphereSkill(SkillBase):
    ...
    def setup(self) -> bool:
        ...
        self.language = self.params.get(
            "language", None
        )  # None means don't include in request
        self.pos_to_expand = self.params.get(
            "pos_to_expand", None
        )  # None means don't include in request
        self.max_synonyms = self.params.get(
            "max_synonyms", None
        )  # None means don't include in request
        ...
        # Build API URL
        self.api_url = (
            f"https://{self.space_name}.signalwire.com/api/datasphere/documents/search"
        )
    ...
    def _search_knowledge_handler(self, args, raw_data):
        ...
        self.logger.info(
            f"DataSphere search requested: '{query}' (document: {self.document_id})"
        )
