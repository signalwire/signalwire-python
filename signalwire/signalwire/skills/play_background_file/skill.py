...
class PlayBackgroundFileSkill(SkillBase):
        ...
        for i, file_obj in enumerate(self.files):
            ...
            for field in required_fields:
                ...
                if not isinstance(file_obj[field], str) or not file_obj[field].strip():
                    raise ValueError(
                        f"File {i} field '{field}' must be a non-empty string"
                    )
            ...
            if not key.replace("_", "").replace("-", "").isalnum():
                raise ValueError(
                    f"File {i} key '{key}' must contain only alphanumeric characters, underscores, and hyphens"
                )
    ...
    def get_tools(self) -> List[Dict[str, Any]]:
        ...
        tool = {
            ...
            "skip_fillers": True,
            "data_map": {"expressions": self._build_expressions()},
        }
