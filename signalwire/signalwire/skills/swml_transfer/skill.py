...
class SWMLTransferSkill(SkillBase):
    ...
    def setup(self) -> bool:
        ...
        for pattern, config in transfers.items():
            if not isinstance(config, dict):
                self.logger.error(
                    f"Transfer config for pattern '{pattern}' must be a dictionary"
                )
                return False
            ...
            if "url" not in config and "address" not in config:
                self.logger.error(
                    f"Transfer config for pattern '{pattern}' must include either 'url' or 'address'"
                )
                return False

            if "url" in config and "address" in config:
                self.logger.error(
                    f"Transfer config for pattern '{pattern}' cannot have both 'url' and 'address'"
                )
                return False
        ...
        self.description = self.params.get(
            "description", "Transfer call based on pattern matching"
        )
        self.parameter_name = self.params.get("parameter_name", "transfer_type")
        self.parameter_description = self.params.get(
            "parameter_description", "The type of transfer to perform"
        )
        self.transfers = transfers
        self.default_message = self.params.get(
    ...
    def register_tools(self) -> None:
        ...
        transfer_tool = (
            ...
            .parameter(
                self.parameter_name, "string", self.parameter_description, required=True
            )
        )
        ...
        for field_name, field_description in self.required_fields.items():
            transfer_tool = transfer_tool.parameter(
                field_name, "string", field_description, required=True
            )
        ...
        for pattern, config in self.transfers.items():
            ...
            result = FunctionResult(
                config["message"], post_process=config["post_process"]
            )
            ...
            if "url" in config:
                ...
                result = result.swml_transfer(
                    config["url"], config["return_message"], config.get("final", True)
                )
            else:
            ...
            transfer_tool = transfer_tool.expression(
                f"${{{f'args.{self.parameter_name}'}}}", pattern, result
            )

        # Add default fallback expression
        default_result = FunctionResult(
            self.default_message, post_process=self.default_post_process
        )
