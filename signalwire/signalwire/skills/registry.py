...
class SkillRegistry:
    ...
    def _load_skill_from_path(
        ...
        if ".." in skill_name or os.sep in skill_name or "/" in skill_name:
            self.logger.error(
                f"Invalid skill name (path traversal attempt): {skill_name}"
            )
            return None
        ...
        try:
            ...
            self.logger.warning(
                f"No skill class found with name '{skill_name}' in {skill_file}"
            )
            return None

        except Exception as e:
            self.logger.error(
                f"Failed to load skill '{skill_name}' from {skill_file}: {e}"
            )
            return None
        ...
        with self._lock:
            ...
            try:
                ...
                if not isinstance(schema, dict):
                    raise ValueError(
                        f"{skill_class.__name__}.get_parameter_schema() must return a dictionary, got {type(schema)}"
                    )

                # Ensure it's not an empty schema (skills should at least have the base parameters)
                if not schema:
                    raise ValueError(
                        f"{skill_class.__name__}.get_parameter_schema() returned an empty dictionary. Skills should at least call super().get_parameter_schema()"
                    )
                ...
                if skill_method and base_method:
                    ...
                    skill_func = (
                        skill_method.__func__
                        if hasattr(skill_method, "__func__")
                        else skill_method
                    )
                    base_func = (
                        base_method.__func__
            ...
            if skill_class.SKILL_NAME in self._skills:
                self.logger.warning(
                    f"Skill '{skill_class.SKILL_NAME}' already registered"
                )
                return
            ...
            try:
                ...
                skills_schema[skill_class.SKILL_NAME] = {
                    ...
                    "supports_multiple_instances": getattr(
                        skill_class, "SUPPORTS_MULTIPLE_INSTANCES", False
                    ),
                ...
                self.logger.error(
                    f"Failed to get schema for skill '{skill_class.SKILL_NAME}': {e}"
                )
            ...
            for entry_point in skill_entries:
                try:
                    ...
                    if issubclass(skill_class, SkillBase):
                        ...
                        if skill_name and (
                            skill_name in builtin_skill_names
                            or skill_name in self._skills
                        ):
                            self.logger.error(
                        ...
                        self.logger.info(
                            f"Loaded skill '{skill_class.SKILL_NAME}' from entry point '{entry_point.name}'"
                        )
                    else:
                        self.logger.warning(
                            f"Entry point '{entry_point.name}' does not provide a SkillBase subclass"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Failed to load skill from entry point '{entry_point.name}': {e}"
                    )
