...
class GoogleMapsClient:
    ...
    def validate_address(self, input_text, bias_lat=None, bias_lng=None):
        ...
        try:
            ...
            if input_text != original:
                logger.debug(
                    f"Normalized spoken numbers: '{original}' → '{input_text}'"
                )

            logger.debug(
                f"validate_address called: input='{input_text}', bias_lat={bias_lat}, bias_lng={bias_lng}"
            )
    ...
    def _nearby_search(self, keyword, lat, lng):
        ...
        name_matches = [
            r for r in data["results"] if keyword_lower in r.get("name", "").lower()
        ]
        ...
        for i, r in enumerate(data["results"][:3]):
            ...
            logger.debug(
                f"  Nearby result #{i + 1}: {r.get('name', '?')} - "
        ...
        if not name_matches:
            logger.debug(
                f"Nearby Search: {len(data['results'])} results but none with '{keyword}' in name"
            )
            return None

        logger.debug(
            f"Nearby Search: {len(name_matches)} name-matched results out of {len(data['results'])}"
        )
        ...
        logger.debug(
            f"Selected closest match: '{name}' at {vicinity} (place_id={place['place_id']})"
        )
    ...
    def _autocomplete_search(self, input_text, bias_lat=None, bias_lng=None):
        ...
        for i, p in enumerate(ac_data["predictions"][:3]):
            logger.debug(
                f"  Autocomplete prediction #{i + 1}: {p.get('description', '?')} (place_id={p['place_id']})"
            )
    ...
    def _get_place_details(self, place_id):
        ...
        logger.debug(
            f"Place Details: name='{name}', formatted='{formatted}', types={types}, "
    ...
    def compute_route(self, origin_lat, origin_lng, dest_lat, dest_lng):
        ...
        try:
            logger.debug(
                f"compute_route called: origin=({origin_lat},{origin_lng}), dest=({dest_lat},{dest_lng})"
            )
            ...
            logger.debug(
                f"Route result: {distance_miles:.1f} miles, {duration_seconds / 60:.0f} min"
            )
            return result
    ...
    def _lookup_address_handler(self, args, raw_data):
        ...
        if not address:
            return FunctionResult(
                "Please provide an address or business name to look up."
            )
        ...
        result = self.client.validate_address(
            address, bias_lat=bias_lat, bias_lng=bias_lng
        )
        if not result:
            return FunctionResult(
