...
class GoogleSearchScraper:
    ...
    def _calculate_content_quality(self, text: str, url: str) -> Dict[str, Any]:
        ...
        if words:
            ...
            diversity_score = min(
                1.0, unique_words / (total_words * 0.4)
            )  # Expect 40% unique
        ...
        boilerplate_penalty = max(
            0, 1.0 - (boilerplate_count * 0.1)
        )  # -10% per boilerplate phrase
    ...
    def search_and_scrape_best(
        ...
        all_text = []
        all_text.append(
            f"Found {len(processed_results)} quality results from {len(search_results)} searched. Showing top {len(best_results)}:\n"
        )
