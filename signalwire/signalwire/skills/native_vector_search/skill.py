...
class NativeVectorSearchSkill(SkillBase):
    ...
    def setup(self) -> bool:
        ...
        self.remote_url = self.params.get(
            "remote_url"
        )  # e.g., "http://user:pass@localhost:8001"
        self.index_name = self.params.get(
            "index_name", "default"
        )  # For remote searches
        ...
        if self.remote_url:
            ...
            try:
                ...
                if response.status_code == 200:
                    self.logger.info(
                        f"Remote search server is available at {self.remote_base_url}"
                    )
        ...
        self.index_nlp_backend = self.params.get(
            "index_nlp_backend", "nltk"
        )  # Default to fast NLTK for indexing
        self.query_nlp_backend = self.params.get(
            "query_nlp_backend", "nltk"
        )  # Default to fast NLTK for search

        # Handle backward compatibility
        if self.nlp_backend is not None:
            self.logger.warning(
                "Parameter 'nlp_backend' is deprecated. Use 'index_nlp_backend' and 'query_nlp_backend' instead."
            )
        ...
        if self.index_nlp_backend not in ["basic", "nltk", "spacy"]:
            self.logger.warning(
                f"Invalid index_nlp_backend '{self.index_nlp_backend}', using 'basic'"
            )
            self.index_nlp_backend = "basic"

        if self.query_nlp_backend not in ["basic", "nltk", "spacy"]:
            self.logger.warning(
                f"Invalid query_nlp_backend '{self.query_nlp_backend}', using 'basic'"
            )
        ...
        if self.build_index and self.source_dir and self.search_available:
            ...
            if self.backend == "sqlite":
                ...
                if self.connection_string and self.collection_name:
                    try:
                        self.logger.info(
                            f"Building pgvector index from {self.source_dir}..."
                        )
                        from signalwire.search import IndexBuilder
                        from signalwire.search.models import resolve_model_alias
                    ...
                    self.logger.warning(
                        "pgvector auto-build requires connection_string and collection_name"
                    )
        ...
        if self.search_available:
            if self.backend == "pgvector":
                ...
                try:
                    ...
                    self.search_engine = SearchEngine(
                        backend="sqlite", index_path=self.index_file
                    )
                    ...
                    self.logger.error(
                        f"Failed to load search index {self.index_file}: {e}"
                    )
                    self.search_available = False
    ...
    def register_tools(self) -> None:
        ...
        search_mode = (
            "remote search server" if self.use_remote else "local document indexes"
        )
    ...
    def _search_handler(self, args, raw_data):
        ...
        try:
            ...
            if not results:
                ...
                if self.response_format_callback and callable(
                    self.response_format_callback
                ):
                    try:
                        callback_context = {
                        ...
                        formatted_response = self.response_format_callback(
                            **callback_context
                        )
            ...
            prefix_postfix_overhead = (
                len(self.response_prefix) + len(self.response_postfix) + 100
            )
            total_overhead = (
                len(results) * estimated_overhead_per_result
            ) + prefix_postfix_overhead
            available_for_content = self.max_content_length - total_overhead

            # Ensure minimum of 500 chars per result
            per_result_limit = (
                max(500, available_for_content // len(results))
                if len(results) > 0
                else 1000
            )
            ...
            for i, result in enumerate(results, 1):
                ...
                if (
                    not tags
                    and "metadata" in result["metadata"]
                    and "tags" in result["metadata"]["metadata"]
                ):
            ...
            if self.response_format_callback and callable(
                ...
                try:
                    ...
                    if isinstance(formatted_response, str):
                        ...
                        self.logger.warning(
                            f"response_format_callback returned non-string type: {type(formatted_response)}"
                        )
