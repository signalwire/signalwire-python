...
class MCPGatewaySkill(SkillBase):
    ...
    def setup(self) -> bool:
        ...
        if not validate_url(self.gateway_url):
            self.logger.error(
                "Gateway URL rejected by SSRF protection: %s", self.gateway_url
            )
            return False
        ...
        for service_config in self.services:
            ...
            try:
                response = self._make_request(
                    "GET", f"{self.gateway_url}/services/{service_name}/tools"
                )
                response.raise_for_status()
                ...
                self.logger.error(
                    f"Failed to get tools for service '{service_name}': {e}"
                )
    ...
    def _call_mcp_tool(
        ...
        if "mcp_call_id" in global_data:
            ...
            self.logger.info(
                f"Using session ID from global_data.mcp_call_id: {session_id}"
            )
        else:
    ...
    def _hangup_handler(
        ...
        if "mcp_call_id" in global_data:
            session_id = global_data["mcp_call_id"]
            self.logger.info(
                f"Cleanup using session ID from global_data.mcp_call_id: {session_id}"
            )
        else:
            session_id = raw_data.get("call_id", "unknown")
        ...
        try:
            response = self._make_request(
                "DELETE", f"{self.gateway_url}/sessions/{session_id}"
            )
            ...
            if response.status_code in [200, 404]:
                ...
                self.logger.warning(
                    f"Failed to cleanup session: HTTP {response.status_code}"
                )
    ...
    def get_global_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "mcp_services": [
                s.get("name") if isinstance(s, dict) else str(s) for s in self.services
            ],
        }
