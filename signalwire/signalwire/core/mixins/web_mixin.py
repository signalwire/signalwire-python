...
class WebMixin(_HostTyped):  # type: ignore[misc]  # _HostTyped is object at runtime; AgentBase under TYPE_CHECKING — intentional split
    ...
    def get_app(self) -> FastAPI:
        ...
        if self._app is None:
            ...
            app = FastAPI(
                redirect_slashes=False, docs_url=None, redoc_url=None, openapi_url=None
            )
            ...
            async def add_security_headers(
                ...
                if getattr(self, "_ssl_enabled", False) or getattr(
    ...
    def serve(self, host: str | None = None, port: int | None = None) -> None:
        ...
        if self._app is None:
            ...
            async def readiness_check() -> Response:
                ...
                ready = (
                    hasattr(self, "_tool_registry")
                    and hasattr(self, "schema_utils")
                    and self.schema_utils is not None
                )

                status_code = 200 if ready else 503
                return Response(
                    content=json.dumps({"status": "ready" if ready else "not_ready"}),
        ...
        username, password, source = self.get_basic_auth_credentials(
            include_source=True
        )
        ...
        if (
            getattr(self, "ssl_enabled", False)
            and getattr(self, "ssl_cert_path", None)
            and getattr(self, "ssl_key_path", None)
        ):
            self.log.info(
                "starting_with_ssl", cert=self.ssl_cert_path, key=self.ssl_key_path
            )
            uvicorn.run(
                self._app,
                host=host,
    ...
    # _read_body_with_limit and _check_content_type were lifted into
    # SWMLService so non-agent SWMLServices that host SWAIG can use them too.
    ...
    async def _handle_root_request(self, request: Request) -> Response:
        ...
        _trust_proxy = getattr(self, "_proxy_url_base_from_env", False) or os.getenv(
        ...
        if forwarded_host and _trust_proxy:
            ...
            if not re.match(r"^[a-zA-Z0-9._-]+(:[0-9]+)?$", forwarded_host):
                self.log.warning(
                    "proxy_header_invalid_host", forwarded_host=forwarded_host
                )
                _valid_proxy = False
            if forwarded_proto not in ("http", "https"):
                self.log.warning(
                    "proxy_header_invalid_proto", forwarded_proto=forwarded_proto
                )
                _valid_proxy = False
            ...
            if _valid_proxy:
                ...
                if not getattr(self, "_proxy_url_base_from_env", False):
                    ...
                    self.log.debug(
                        "proxy headers present but keeping env proxy URL",
            ...
            if not getattr(self, "_proxy_url_base_from_env", False):
                self.log.debug(
                    "No proxy headers found, clearing proxy URL",
                    proxy_url_base_from_env=getattr(
            ...
            if hasattr(super(), "_detect_proxy_from_request"):
                ...
                if hasattr(super(), "_proxy_url_base") and getattr(
    ...
    async def _handle_debug_request(self, request: Request) -> Response:
        ...
        req_log = self.log.bind(
            endpoint="debug", method=request.method, path=request.url.path
        )
    ...
    async def _handle_check_for_input_request(
        ...
        try:
            ...
            if conversation_id and (
                len(conversation_id) > 256
                or not all(c.isalnum() or c in "-_." for c in conversation_id)
            ):
                return Response(
