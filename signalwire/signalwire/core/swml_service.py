...
class SWMLService(ToolMixin):
    ...
    def __init__(
        ...
        if self._proxy_url_base:
            self.log.warning(
                "SWML_PROXY_URL_BASE is set in environment - This overrides SSL configuration and port settings. Remove this variable to use automatic detection.",
                proxy_url_base=self._proxy_url_base,
            )
        self._proxy_detection_done = False
        self._proxy_debug = os.environ.get("SWML_PROXY_DEBUG", "").lower() in (
            "true",
            "1",
            "yes",
        )
        ...
        self.schema_utils = SchemaUtils(
            schema_path, schema_validation=self._schema_validation
        )
    ...
    def _build_webhook_url(
        ...
        return self._build_full_url(
            endpoint=endpoint, include_auth=True, query_params=query_params
        )
        ...
        if forwarded:
            ...
            try:
                ...
                if host_part:
                    ...
                    proto = (
                        proto_part.split("=", 1)[1].strip('"') if proto_part else "http"
                    )
