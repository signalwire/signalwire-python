...
class AgentBase(  # type: ignore[misc]  # intentional diamond: WebMixin's serve/_proxy_url_base override SWMLService/ServerlessMixin's by MRO order; mypy flags the base-vs-base shape diff but the resolution is deliberate
    ...
    def get_full_url(self, include_auth: bool = False) -> str:
        ...
        if mode == "cgi":
            ...
            if project_id:
                base_url = (
                    f"https://{region}-{project_id}.cloudfunctions.net/{service_name}"
                )
            else:
            ...
            if function_app_name:
                base_url = (
                    f"https://{function_app_name}.azurewebsites.net/api/{function_name}"
                )
            else:
    ...
    def enable_sip_routing(
        self, auto_map: bool = True, path: str = "/sip"
    ) -> "AgentBase":
        """
        ...
        def sip_routing_callback(
            ...
            if sip_username:
                ...
                if (
                    hasattr(self, "_sip_usernames")
                    and sip_username.lower() in self._sip_usernames
                ):
    ...
    def _render_swml(
        ...
        self.log.debug(
            ...
            use_ephemeral=bool(
                modifications and modifications.get("__use_ephemeral_agent")
            ),
        ...
        if modifications and modifications.get("__use_ephemeral_agent"):
            ...
            self.log.debug(
                "creating_ephemeral_agent",
                original_sections=len(self._prompt_manager._sections)
            ...
            self.log.debug(
                ...
                ephemeral_sections=len(agent_to_use._prompt_manager._sections)
                if hasattr(agent_to_use._prompt_manager, "_sections")
                else 0,
            )
            ...
            if self._dynamic_config_callback:
                try:
                    ...
                    self.log.debug(
                        "calling_dynamic_config_on_ephemeral", has_request=bool(request)
                    )
                    self._dynamic_config_callback(
                        query_params, body_params, headers, agent_to_use
                    )
                    self.log.debug(
                        "dynamic_config_complete",
                        ephemeral_sections_after=len(
                            agent_to_use._prompt_manager._sections
                        )
                        if hasattr(agent_to_use._prompt_manager, "_sections")
                        else 0,
                    )
        ...
        query_params = (
            agent_to_use._swaig_query_params.copy()
            if agent_to_use._swaig_query_params
            else {}
        )
        ...
        # Use override if set
        if (
            hasattr(agent_to_use, "_web_hook_url_override")
            and agent_to_use._web_hook_url_override
        ):
        ...
        if (
            hasattr(agent_to_use, "_internal_fillers")
            and agent_to_use._internal_fillers
        ):
        ...
        self.log.debug(
            ...
            function_count=len(agent_to_use._tool_registry._swaig_functions)
            if hasattr(agent_to_use._tool_registry, "_swaig_functions")
            else 0,
            functions=list(agent_to_use._tool_registry._swaig_functions.keys())
            if hasattr(agent_to_use._tool_registry, "_swaig_functions")
            else [],
        )
        ...
        if not hasattr(agent_to_use._tool_registry, "_swaig_functions"):
            self.log.warning(
                "tool_registry_missing_swaig_functions",
                registry_id=id(agent_to_use._tool_registry),
                agent_id=id(agent_to_use),
            )
        ...
        for name, func in agent_to_use._tool_registry._swaig_functions.items():
            if isinstance(func, dict):
                ...
                if func.secure and call_id:
                    token = agent_to_use._create_tool_token(
                        tool_name=name, call_id=call_id
                    )
                    self.log.debug(
                        "created_token_for_function",
                        function=name,
                        call_id=call_id,
                ...
                if hasattr(func, "wait_file") and func.wait_file:
                    ...
                    if wait_file_url and not wait_file_url.startswith(
                        ("http://", "https://", "//")
                    ):
                ...
                if (
                    hasattr(func, "wait_file_loops")
                    and func.wait_file_loops is not None
                ):
                ...
                if hasattr(func, "webhook_url") and func.webhook_url:
                    ...
                    function_entry["web_hook_url"] = agent_to_use._build_webhook_url(
                        "swaig", url_params
                    )
        ...
        if functions:
            ...
            if "defaults" not in swaig_obj:
                swaig_obj["defaults"] = {"web_hook_url": default_webhook_url}
        ...
        if post_prompt:
            ...
            if call_id and hasattr(agent_to_use, "_session_manager"):
                try:
                    token = agent_to_use._session_manager.create_tool_token(
                        "post_prompt", call_id
                    )
                    if token:
                        query_params["__token"] = (
                            token  # Use __token to avoid collision
                        )
                except Exception as e:
                    agent_to_use.log.error(
                        "post_prompt_token_creation_error", error=str(e)
                    )

            # Build the URL with the token (if any)
            post_prompt_url = agent_to_use._build_webhook_url(
                "post_prompt", query_params
            )

            # Use override if set
            if (
                hasattr(agent_to_use, "_post_prompt_url_override")
                and agent_to_use._post_prompt_url_override
            ):
        ...
        if ai_handler:
            try:
                ...
                if getattr(agent_to_use, "_debug_events_enabled", False):
                    ...
                    agent_to_use._params["debug_webhook_url"] = (
                        agent_to_use._build_webhook_url(
                            "debug_events", debug_query_params
                        )
                    )
                    agent_to_use._params["debug_webhook_level"] = getattr(
                        agent_to_use, "_debug_events_level", 1
                    )
    ...
    def _build_webhook_url(
        ...
        if mode != "server":
            ...
            if query_params:
                ...
                filtered_params = {
                    k: v for k, v in query_params.items() if k != "call_id" and v
                }
                if filtered_params:
    ...
    def _find_summary_in_post_data(self, body, logger):
        ...
        if "post_prompt_data" in body:
            ...
            if isinstance(pdata, dict):
                if (
                    "parsed" in pdata
                    and isinstance(pdata["parsed"], list)
                    and pdata["parsed"]
                ):
    ...
    def _create_ephemeral_copy(self):
        ...
        ephemeral_agent._post_prompt_llm_params = copy.deepcopy(
        ...
        if hasattr(self, "_contexts_builder") and self._contexts_builder:
            ...
            ephemeral_agent._contexts_builder = copy.deepcopy(
                self._contexts_builder, memo
            )
        ...
        ephemeral_agent._prompt_manager._post_prompt_text = copy.deepcopy(
            self._prompt_manager._post_prompt_text
        )
        ephemeral_agent._prompt_manager._contexts = copy.deepcopy(
            self._prompt_manager._contexts
        )
    ...
    async def _handle_request(self, request: Request, response: Response):
        ...
        try:
            ...
            # Get call_id
            call_id = (
                body.get("call_id") if body else request.query_params.get("call_id")
            )
