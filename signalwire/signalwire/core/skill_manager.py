    ...
    def load_skill(
        ...
        if skill_class is None:
            try:
                ...
                error_msg = (
                    f"Skills registry not available. Cannot load skill '{skill_name}'"
                )
                self.logger.error(error_msg)
                return False, error_msg
        ...
        try:
            ...
            missing_env_vars = [
                var for var in skill_instance.REQUIRED_ENV_VARS if not os.getenv(var)
            ]
            if missing_env_vars:
                error_msg = (
            ...
            self.logger.info(
                f"Successfully loaded skill instance '{instance_key}' (skill: '{skill_name}')"
            )
            return True, ""
