...
class SecurityConfig:
    ...
    def _set_defaults(self) -> None:
        ...
        self.ssl_cert_path: str | None = None
        self.ssl_key_path: str | None = None
        ...
        # Authentication
        self.basic_auth_user: str | None = None
        self.basic_auth_password: str | None = None
        # Where basic_auth_password came from: "environment", "config file"
        # or "generated". None until one of them sets it.
        self.basic_auth_source: str | None = None

    def _load_config_file(
        self, config_file: str | None, service_name: str | None
    ) -> None:
        ...
        if isinstance(auth_config, dict):
            ...
            if isinstance(basic_auth, dict):
                ...
                if "password" in basic_auth:
                    ...
                    self.basic_auth_source = "config file"
    ...
    def load_from_env(self) -> None:
        ...
        self.ssl_verify_mode = os.environ.get(
            self.SSL_VERIFY_MODE, self.DEFAULTS[self.SSL_VERIFY_MODE]
        )

        # Additional security settings
        self.allowed_hosts = self._parse_list(
            os.environ.get(self.ALLOWED_HOSTS, self.DEFAULTS[self.ALLOWED_HOSTS])
        )
        self.cors_origins = self._parse_list(
            os.environ.get(self.CORS_ORIGINS, self.DEFAULTS[self.CORS_ORIGINS])
        )
        self.max_request_size = int(
            os.environ.get(self.MAX_REQUEST_SIZE, self.DEFAULTS[self.MAX_REQUEST_SIZE])
        )
        self.rate_limit = int(
            os.environ.get(self.RATE_LIMIT, self.DEFAULTS[self.RATE_LIMIT])
        )
        self.request_timeout = int(
            os.environ.get(self.REQUEST_TIMEOUT, self.DEFAULTS[self.REQUEST_TIMEOUT])
        )

        # HSTS settings
        use_hsts_env = os.environ.get(self.USE_HSTS, "").lower()
        self.use_hsts = (
            use_hsts_env != "false" if use_hsts_env else self.DEFAULTS[self.USE_HSTS]
        )
        self.hsts_max_age = int(
            os.environ.get(self.HSTS_MAX_AGE, self.DEFAULTS[self.HSTS_MAX_AGE])
        )

        # Authentication
        self.basic_auth_user = os.environ.get(self.BASIC_AUTH_USER)
        self.basic_auth_password = os.environ.get(self.BASIC_AUTH_PASSWORD)
        self.basic_auth_source = "environment" if self.basic_auth_password else None
    ...
    def get_basic_auth(self) -> tuple[str, str]:
        ...
        if not self.basic_auth_password:
            ...
            self.basic_auth_source = "generated"
            if not getattr(self, "_basic_auth_autogen_warned", False):
                logger.warning(
                    "basic_auth_password_autogenerated",
                    username=username,
                    hint=(
                        "No SWML_BASIC_AUTH_PASSWORD found in environment and "
                        "no password passed to the agent constructor. The SDK "
                        "generated a random password that exists only in this "
                        "process; external callers will get HTTP 401 unless "
                        "they read the value from this process's env. To fix, "
                        "set SWML_BASIC_AUTH_USER and SWML_BASIC_AUTH_PASSWORD "
                        "in your .env (and load_dotenv() before constructing "
                        "the agent), or pass basic_auth=(user, pass) to "
                        "AgentBase()."
                    ),
                )
                self._basic_auth_autogen_warned = True
        else:
    ...
    def log_config(self, service_name: str) -> None:
