...
The gateway sits between the browser and the chat service, so only it ever holds the project token:
...
Create the gateway and mount it on your agent's app:
...
It mounts on any FastAPI app, not only an agent:

<!-- snippet: no-run illustrative fragment (references `gateway` established in the surrounding prose, and needs `from fastapi import FastAPI`) -->
```python
app = FastAPI()
...
**First message:** there's no handle yet, so the gateway mints a conversation:
...
The response carries the new handle in a header:
...
The gateway creates the conversation and returns its greeting:
...
The response carries the dialogue and the three fields described next:
...
accepted and goes nowhere. Send it on every call regardless, since that is
what guarantees the creating one carries it, but design for a snapshot taken
at the greeting, not a feed of where the visitor is now.
...
not stop anyone using curl, who omits the header. Treat it as leak
containment, not access control: the caps in
[What a Stolen Key Can Do](#what-a-stolen-key-can-do) are what actually bound
your exposure.
...
The constructor accepts these keyword-only parameters:
